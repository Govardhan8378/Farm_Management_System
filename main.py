...
st.set_page_config(
    page_title="Farm Management System",  # Title of the app
    page_icon="🌾",  # You can use an emoji or a file path
    layout="wide"
)
...
if not st.session_state["authenticated"]:
    ...
    if menu == translate_text("Login"):
        ...
        # Set default demo credentials (for testing/demo purpose)
        default_username = "demo_user"
        default_password = "demo123"

        # Pre-fill the input fields with default values
        username = st.text_input(translate_text("Username"), value=default_username)
        password = st.text_input(translate_text("Password"), value=default_password, type="password")

    if st.button(translate_text("Login")):
        
        if default_username=="demo_user" and default_password=="demo123":
        ...
        reset_phone = st.text_input("Enter your Phone Number")
    ...
    # Sidebar User Info
    with st.sidebar:
       st.sidebar.success(f"👤 {translate_text('Logged in as')} **{['username']}**")
       st.sidebar.info(f"🌾 {translate_text('Acres Owned')}: **{['acres']}**")
       st.sidebar.info(f"📞 {translate_text('Contact')}: **{['phone']}**")
    ...
    crop_details = {
        "Lentil": {"desc": "Lentil is a cool-season crop that prefers well-drained soil.", "img": "images/lentil.jpg"},
        "Bajra": {"desc": "Bajra (Pearl Millet) is a drought-resistant crop grown in dry regions.", "img": "images\\Bajra.jpg"},
        "Barley": {"desc": "Barley is a versatile crop used for food, beer production, and fodder.", "img": "images/Barley.webp"},
        "Chickpea": {"desc": "Chickpea is a protein-rich legume commonly grown in arid regions.", "img": "images/Chickpea.jpg"},
        "Coconut": {"desc": "Coconut is a tropical tree crop used for oil, water, and fiber.", "img": "images/Coconut.png"},
        "Coffee": {"desc": "Coffee is a globally consumed beverage crop grown in hilly areas.", "img": "images/Coffee.jpg"},
        "Cotton": {"desc": "Cotton is a fiber crop used in textiles and garment production.", "img": "images/Cotton.jpg"},
        "Fruits": {"desc": "Fruits provide essential vitamins and are grown in different climates.", "img": "images/Fruits.webp"},
        "Groundnut": {"desc": "Groundnut (Peanut) is an oilseed crop rich in protein and fat.", "img": "images/Groundnut.webp"},
        "Jowar": {"desc": "Jowar (Sorghum) is a drought-tolerant cereal crop.", "img": "images/Jowar.webp"},
        "Jute": {"desc": "Jute is a fiber crop used for making ropes, mats, and bags.", "img": "images/Jute.jpg"},
        "Maize": {"desc": "Maize (Corn) is a staple cereal crop grown worldwide.", "img": "images/Maize.jpeg"},
        "Millets": {"desc": "Millets are small-seeded cereals known for their nutritional benefits.", "img": "images/Millets.webp"},
        "Mustard": {"desc": "Mustard is an oilseed crop used for cooking oil and condiments.", "img": "images/Mustard.png"},
        "Peas": {"desc": "Peas are nutritious legumes grown in cool weather.", "img": "images/Peas.jpg"},
        "Pulses": {"desc": "Pulses are edible seeds of leguminous plants, rich in protein.", "img": "images/Pulses.webp"},
        "Rice": {"desc": "Rice is a staple food crop requiring high water availability.", "img": "images/Rice.jpg"},
        "Soybean": {"desc": "Soybean is an oilseed crop used in food and industrial applications.", "img": "images/Soybean.jpg"},
        "Sugarcane": {"desc": "Sugarcane is a tropical grass used for sugar and ethanol production.", "img": "images/Sugarcane.jpg"},
        "Sunflower": {"desc": "Sunflower is an oilseed crop known for its edible oil.", "img": "images/Sunflower.jpg"},
        "Tea": {"desc": "Tea is a popular beverage crop grown in hilly regions.", "img": "images/Tea.jpg"},
        "Tobacco": {"desc": "Tobacco is a commercial crop used for cigarettes and chewing products.", "img": "images/Tobacco.jpg"},
        "Vegetables": {"desc": "Vegetables are rich in vitamins and minerals, essential for health.", "img": "images/Vegetables.webp"},
        "Wheat": {"desc": "Wheat is a major staple crop used for making flour and bread.", "img": "images/Wheat.jpg"}
    }
    ...
    with st.sidebar:
        selected = option_menu(
            menu_title="Features",  
            options=["Home", "🌾 Crop Recommendation System", "🌱 Fertilizer Recommendation System", "📊 Telangana Crop Market Predictor","Market Visualization","Chatbot"],  
            icons=["menu-app", "seedling", "leaf","tag","graph-up","chat-fill"],  
            menu_icon="list",  
            default_index=0  
        )

    if selected == "📊 Telangana Crop Market Predictor":
        st.title("📊 Telangana Crop Market Predictor")
        st.write("Select a crop and season to predict the market price, best selling location, and the nearest industry.")
        ...
        if st.button("Predict Crop Market Details"):
            try:
                ...
                # Display the results
                st.success(f"💰 Estimated Selling Price: ₹ {round(predicted_price, 2)} per quintal")
                st.info(f"📍 Recommended Selling Location: {predicted_location}")
                st.warning(f"🏭 Nearest Industry Name: {predicted_industry}")

            except Exception as e:
                st.error(f"Error occurred: {e}")

    elif selected == "🌱 Fertilizer Recommendation System":
        # Load trained model and encoders for fertilizer recommendation
        model = joblib.load("fertilizer_model.pkl")
        label_encoders = joblib.load("label_encoders.pkl")

        st.title("🌱 Fertilizer Recommendation System")
        st.markdown("### Enter the soil and climate details to get the best fertilizer recommendation")

        # Input fields for soil and crop selection
        soil_type = st.selectbox("Select Soil Type", label_encoders["soiltype"].classes_)
        crop_name = st.selectbox("Select Crop Name", label_encoders["cropname"].classes_)

        st.markdown("#### **Nutrient & Climate Conditions**")
    ...
    elif selected == "🌾 Crop Recommendation System":
